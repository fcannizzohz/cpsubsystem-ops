...
| Panel | Metric | What to watch |
|-------|--------|---------------|
| Lock Activity Rate | `changes(hz_cp_lock_lockCount[1m])` | Lock state changes per minute (÷2 ≈ acquisitions); high = contention bottleneck |
...
| Lock activity rate | `changes(hz_cp_lock_lockCount[1m])` | Lock state changes per lock name (÷2 ≈ acquisitions) |
