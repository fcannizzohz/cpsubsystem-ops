...
- [ ] **Alert: semaphore permits exhausted** — fire when `hz_cp_semaphore_available == 0` for > N seconds. Clients will block. `[metric exists]`
...
- [ ] **Alert: reachable members below quorum** — fire when `max(activeMembers) − max(missingMembers) < 3`. `[metric exists]`
...
- [ ] **Fix Prometheus recording rules** — `cp-subsystem.yml` uses `hazelcast_raft_*` (V2 names that do not exist). Correct to `hz_raft_*` (V1). Recording rules and derived alerts currently produce no data.
